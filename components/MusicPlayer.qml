...
Rectangle {
    ...
    property string songTitle: theme.localized("Unknown song", "未知歌曲")
    property string artistName: theme.localized("Unknown artist", "未知艺术家")
    ...
    Accessible.name: root.songTitle + " — " + root.artistName

    function localizedSongTitle(value) {
        const normalized = String(value || "").trim();
        return normalized.length === 0 || normalized === "未知歌曲" || normalized.toLowerCase() === "unknown song"
                ? theme.localized("Unknown song", "未知歌曲")
                : normalized;
    }

    function localizedArtistName(value) {
        const normalized = String(value || "").trim();
        return normalized.length === 0 || normalized === "未知艺术家" || normalized.toLowerCase() === "unknown artist"
                ? theme.localized("Unknown artist", "未知艺术家")
                : normalized;
    }
    function playSourcePath(p) {
        ...
        if (musicLibrary && musicLibrary.isValidMusicFile && !musicLibrary.isValidMusicFile(src))
            return;
        if (root.playlistModelRef && typeof root.playlistModelRef.get === "function") {
            ...
            for (var i = 0; i < root.playlistModelRef.count; i++) {
                ...
                if (it && (it.source === src || it.source === p)) {
                    found = i;
                    break;
                }
            }
    ...
    function addSources(sources) {
        if (!sources || !playlistModel)
            return;
        // Index existing rows once so importing many files stays linear.
        const knownSources = Object.create(null);
        for (var existingIndex = 0; existingIndex < playlistModel.count; ++existingIndex) {
            const existingItem = playlistModel.get(existingIndex);
            if (existingItem && existingItem.source)
                knownSources[String(existingItem.source)] = true;
        }
        for (var i = 0; i < sources.length; i++) {
            var s = sources[i];
            if (typeof s === 'string' && s.indexOf('file:///') === 0)
                s = decodeURIComponent(s.substring(8));
            const sourceKey = String(s || "");
            if (sourceKey.length > 0 && !knownSources[sourceKey]) {
                playlistModel.append({
                    source: sourceKey
                });
                knownSources[sourceKey] = true;
            }
        }
    ...
    function resetSources(sources) {
        if (!playlistModel)
            return;
        updatePlaylistFromFiles(sources || []);
    }
    ...
    function stopPlayback() {
        if (mediaPlayer)
            mediaPlayer.stop();
        root.source = "";
        root.songTitle = theme.localized("Unknown song", "未知歌曲");
        root.artistName = theme.localized("Unknown artist", "未知艺术家");
        root.coverImage = "";
    ...
    // Verbose media events are opt-in so release builds do not serialize
    // console output on the GUI thread during scans and state transitions.
    ...
    AudioMetadata {
        ...
        onTitleChanged: {
            root.songTitle = root.localizedSongTitle(title);
        }

        onArtistChanged: {
            root.artistName = root.localizedArtistName(artist);
        }
        ...
        onCoverImageUrlChanged: {
            ...
            if (url.length === 0) {
                ...
                // The C++ metadata reader creates a unique temporary path for
                // each cover. Reusing that URL lets the two Image nodes share
                // the decoded resource without stale-cache query strings.
                root.coverImage = url;
            }
        }
    ...
    Item {
        ...
        MouseArea {
            ...
            Accessible.name: theme.localized("Open now playing", "打开正在播放")
            onClicked: {
        ...
        // 阴影效果
        layer.enabled: root.shadowEnabled && root.backgroundVisible && !(theme && theme.highContrast)
            ...
            RowLayout {
                ...
                Row {
                    ...
                    Text {
                        ...
                        MouseArea {
                            ...
                            cursorShape: Qt.PointingHandCursor
                            Accessible.role: Accessible.Button
                            Accessible.name: theme.localized("Previous track", "上一曲")
                            onClicked: {
                                // 触发上一曲信号
                    ...
                    Text {
                        ...
                        MouseArea {
                            ...
                            cursorShape: Qt.PointingHandCursor
                            Accessible.role: Accessible.Button
                            Accessible.name: root.isPlaying
                                             ? theme.localized("Pause", "暂停")
                                             : theme.localized("Play", "播放")
                    ...
                    Text {
                        ...
                        MouseArea {
                            ...
                            cursorShape: Qt.PointingHandCursor
                            Accessible.role: Accessible.Button
                            Accessible.name: theme.localized("Next track", "下一曲")
                            onClicked: {
                                // 触发下一曲信号
                                root.nextClicked();
                            }
                ...
                Item {
                    ...
                    MouseArea {
                        ...
                        cursorShape: Qt.PointingHandCursor
                        Accessible.role: Accessible.Button
                        Accessible.name: root.playMode === 0
                                         ? theme.localized("Repeat all", "列表循环")
                                         : (root.playMode === 1
                                            ? theme.localized("Repeat one", "单曲循环")
                                            : theme.localized("Shuffle", "随机播放"))
                        onClicked: {
    ...
    // Populate large scan results in bounded batches. ListModel mutations are
    // GUI-thread work, so yielding every 16 ms keeps input and rendering live.
    property var pendingPlaylistSources: []
    property int pendingPlaylistIndex: 0
    property int playlistBatchSize: 64

    Timer {
        id: playlistPopulateTimer
        interval: 16
        repeat: true
        onTriggered: root.populatePlaylistBatch()
    }
    ...
    function loadProjectPlaylist() {
        // Directory walks run on the bounded music-library worker pool. The
        // existing musicFilesChanged handler applies the result on the UI thread.
        musicLibrary.scanAllAvailableMusicAsync(true);
    ...
    function updatePlaylistFromFiles(files) {
        playlistPopulateTimer.stop();
        pendingPlaylistSources = files ? Array.prototype.slice.call(files) : [];
        pendingPlaylistIndex = 0;
        playlistModel.clear();
        if (pendingPlaylistSources.length === 0) {
            root.finishPlaylistUpdate();
            return;
        }
        playlistPopulateTimer.start();
    }

    function populatePlaylistBatch() {
        const endIndex = Math.min(pendingPlaylistSources.length,
                                  pendingPlaylistIndex + Math.max(1, playlistBatchSize));
        for (var i = pendingPlaylistIndex; i < endIndex; ++i) {
            playlistModel.append({
                source: pendingPlaylistSources[i]
            });
        }
        pendingPlaylistIndex = endIndex;
        if (pendingPlaylistIndex >= pendingPlaylistSources.length) {
            playlistPopulateTimer.stop();
            root.finishPlaylistUpdate();
        }
    }

    function finishPlaylistUpdate() {
        pendingPlaylistSources = [];
        pendingPlaylistIndex = 0;
    ...
    Connections {
        ...
        function onEffectiveLanguageChanged() {
            root.songTitle = root.localizedSongTitle(root.songTitle);
            root.artistName = root.localizedArtistName(root.artistName);
        }
    }
