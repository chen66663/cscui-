...
/*
  NextHolidayCountdown.qml
  最近节假日到来时间组件：显示最近的节假日日期（MM/DD + 星期几）、名称与剩余天数。
  颜色风格参考 Clock：theme.secondaryColor、theme.textColor、theme.focusColor。
...
Item {
    ...
    property bool useNetwork: false
    ...
    property var holidaysFallback: [
        {
            name: theme.localized("New Year's Day", "元旦"),
            month: 1,
            day: 1
        },
        {
            name: theme.localized("Labour Day", "劳动节"),
            month: 5,
            day: 1
        },
        {
            name: theme.localized("National Day", "国庆节"),
            month: 10,
            day: 1
        }
    ]
    ...
    function weekdayStr(d) {
        const arr = theme.isChinese
                    ? ["周日", "周一", "周二", "周三", "周四", "周五", "周六"]
                    : ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
        return arr[d.getDay()];
    }

    function localizedHolidayName(name) {
        const value = String(name || "");
        if (value === "元旦" || value === "New Year's Day")
            return theme.localized("New Year's Day", "元旦");
        if (value === "劳动节" || value === "Labour Day" || value === "Labor Day")
            return theme.localized("Labour Day", "劳动节");
        if (value === "国庆节" || value === "National Day")
            return theme.localized("National Day", "国庆节");
        return value;
    }

    function findNextHoliday(refDate) {
        const y = refDate.getFullYear();
        const today = new Date(y, refDate.getMonth(), refDate.getDate());
        let best = null;
        for (let i = 0; i < holidays.length; i++) {
            ...
            const diffDays = Math.round((candidateUtc - todayUtc) / (24 * 3600 * 1000));
    ...
    function fetchHolidays() {
        ...
        const requestUrl = apiUrl + separator + "year=" + encodeURIComponent(apiYear) + "&country=" + encodeURIComponent(countryCode);
                ...
                if (xhr.status === 200) {
                    try {
                        ...
                        if (parsed.length > 0) {
                            ...
                            holidays = holidaysFallback;
                            errorMessage = theme.localized("No holiday data was returned; using the local list.", "未获取到节日数据，已使用本地列表。");
                        }
                    } catch (e) {
                        holidays = holidaysFallback;
                        errorMessage = theme.localized("Holiday data could not be parsed; using the local list.", "解析节日数据失败，已使用本地列表。");
                    }
                } else {
                    holidays = holidaysFallback;
                    errorMessage = theme.localized("Holiday request failed (", "节日接口请求失败（")
                                   + xhr.status
                                   + theme.localized("); using the local list.", "），已使用本地列表。")
                }
                updateNextHolidayInfo();
            }
        };
    ...
    readonly property string nameLine: (nextHolidayInfo ? localizedHolidayName(nextHolidayInfo.name) : "")
    readonly property string remainLine: nextHolidayInfo
                                                  ? (theme.isChinese
                                                     ? "还有 " + nextHolidayInfo.diffDays + " 天"
                                                     : nextHolidayInfo.diffDays + (nextHolidayInfo.diffDays === 1 ? " day left" : " days left"))
                                                  : ""
    ...
    Timer {
        ...
        onTriggered: {
            root.now = new Date();
            if (root.now.getFullYear() !== apiYear)
                apiYear = root.now.getFullYear();
            updateNextHolidayInfo();
        }
    }
    ...
    Connections {
        ...
        function onEffectiveLanguageChanged() {
            if (!root.useNetwork) {
                root.holidays = root.holidaysFallback;
                root.updateNextHolidayInfo();
            }
        }
    }
    ...
    Row {
        ...
        Column {
            ...
            Text {
                text: theme.localized("Next holiday", "最近节假日")
                color: textColor
                font.pixelSize: 14
                font.bold: true
            }
        ...
        Item {
            ...
            Connections {
                target: root
                function onNextHolidayInfoChanged() {
                    ring.requestPaint();
                }
            }
