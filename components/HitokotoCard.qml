...
Item {
    ...
    // Explicit theme injection keeps the card usable outside the shared
    // application context while retaining a local fallback for standalone use.
    ...
    // === 一言数据 ===
    property bool useNetworkQuote: false
    property string quoteApiUrl: "https://v1.hitokoto.cn/?encode=json"
    property string quoteText: theme ? theme.localized("Clarity is a feature.", "清晰本身就是一种功能。") : "Clarity is a feature."
    property string quoteFrom: "cscui"
    property string quoteFromWho: ""
    // 每 12 小时刷新一次（单位：毫秒）
    property int quoteRefreshIntervalMs: 12 * 60 * 60 * 1000

    // === 背景图片源（改为 loliapi 随机图片） ===
    property bool useNetworkImage: false
    property string acgApiUrl: ""
    property string bingImageUrl: "" // 实际使用 acgApiUrl 返回的图片
    property string imageDirectUrl: "" // 记录最终解析出的直链，用于外部跳转
    readonly property string currentImageUrl: imageDirectUrl.length > 0 ? imageDirectUrl : bingImageUrl
    // 每天刷新一次背景（单位：毫秒）
    property int bingRefreshIntervalMs: 24 * 60 * 60 * 1000

    // 阴影（仅作用于背景容器）
    layer.enabled: shadowEnabled && backgroundVisible && !(theme && theme.highContrast)
    ...
    Item {
        ...
        Image {
            ...
            onStatusChanged: {
                if (status === Image.Error && root.useNetworkImage) {
        ...
        LoadingIndicator {
            ...
            // The default card is intentionally offline. Do not leave the
            // spinner running for an empty URL when network imagery is off.
            visible: root.backgroundVisible
                     && root.useNetworkImage
                     && (bingSource.status !== Image.Ready || root.bingImageUrl === "")
            running: visible
            z: 1.5
        }
    ...
    Item {
        ...
        MouseArea {
            anchors.fill: parent
            z: 3
            enabled: root.useNetworkQuote || root.useNetworkImage
            hoverEnabled: true
        ...
        Text {
            id: openImageIcon
            // Avoid `(stringA || stringB).length`: Qt 6.9 qmlcachegen lowers
            // that expression to a primitive lookup that can crash in AOT
            // builds while this asynchronously incubated page is finalized.
            visible: root.currentImageUrl.length > 0
            ...
            MouseArea {
                ...
                onClicked: Qt.openUrlExternally(root.currentImageUrl)
            }
        }
    }
                ...
                if (xhr.readyState === XMLHttpRequest.DONE) {
                    if (xhr.status === 200) {
                        try {
                            ...
                            root.quoteText = payload.hitokoto || (theme ? theme.localized("(No content)", "（无内容）") : "(No content)")
    ...
    function fetchBingImage() {
        if (!root.useNetworkImage || root.acgApiUrl.length === 0)
            return
    ...
    Timer {
        ...
        running: root.visible && useNetworkQuote
        repeat: true
    ...
    Timer {
        ...
        running: root.visible && useNetworkImage
        repeat: true
    ...
    Component.onCompleted: {
        ...
        if (useNetworkImage) fetchBingImage()
    }
