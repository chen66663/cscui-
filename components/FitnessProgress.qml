...
Rectangle {
    ...
    antialiasing: true
    // 卡片阴影
    layer.enabled: root.shadowEnabled && !(theme && theme.highContrast)
    ...
    MultiEffect {
        ...
        shadowColor: Qt.rgba(root.progressColor.r, root.progressColor.g, root.progressColor.b, 0.35)
        shadowBlur: 8
    ...
    Text {
        ...
        color: theme.textColor
        font.pixelSize: Math.max(24, Math.min(36, root.width * 0.15))
        font.bold: true
        opacity: 0.98
    }
    ...
    onProgressColorChanged: arcCanvas.requestPaint()
    onTrackColorChanged: arcCanvas.requestPaint()
