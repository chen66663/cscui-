...
// Single-selection group. The public selectedIndices array is retained for
// compatibility with existing consumers, although at most one index is stored.
FocusScope {
    id: root
    ...
    function updateMaxTextWidth() {
        ...
        for (var i = 0; i < root.itemCount; ++i) {
            var value = root.itemAt(i);
            measureText.text = String(value && value.text !== undefined ? value.text : value);
    ...
    function chooseIndex(index) {
        if (!root.enabled)
            return;
        var next = root.selectedIndices.length > 0 && root.selectedIndices[0] === index ? [index] : [index];
        root.selectedIndices = next;
        root.selectionChanged(next, [root.itemAt(index)]);
    }
