...
import cscui 1.0
    ...
    // 音乐窗口作用域数据（外部 AnimatedWindow 传入）
    property string coverImage: ""
    property bool coverIsDefault: false
    property string title: theme ? theme.localized("Unknown song", "未知歌曲") : "Unknown song"
    property string artist: theme ? theme.localized("Unknown artist", "未知艺术家") : "Unknown artist"
    ...
    // 歌词数据：解析自 LRC，[{ t:毫秒, text:字符串 }]
        ...
        property string loadedLyricsSource: ""
    ...
    property int coverFadeDuration: theme && theme.reducedMotion ? 0 : 320
    ...
    function localizedSongTitle(value) {
        ...
        return normalized.length === 0 || normalized === "未知歌曲" || normalized.toLowerCase() === "unknown song"
                ? (theme ? theme.localized("Unknown song", "未知歌曲") : "Unknown song")
                : normalized;
    }
    ...
    onSourceItemChanged: {
        if (!sourceItem) {
            musicLib.loadLyricsTextAsync("")
            title = theme ? theme.localized("Unknown song", "未知歌曲") : "Unknown song"
            artist = theme ? theme.localized("Unknown artist", "未知艺术家") : "Unknown artist"
        ...
        if (typeof sourceItem.songTitle === "string") title = localizedSongTitle(sourceItem.songTitle)
        if (typeof sourceItem.artistName === "string") artist = localizedArtistName(sourceItem.artistName)
    ...
    MusicLibrary {
        ...
        onLyricsReady: function(source, text, filePath) {
            if (source.toString() !== root.loadedLyricsSource)
                return
            root.applyLyricsResult(text, filePath)
        }
    }
    ...
    function loadLyricsForSource(src) {
        if (!src || src.length === 0) {
            // Empty input also cancels an in-flight worker request when the
            // full-screen player is detached.
            musicLib.loadLyricsTextAsync("")
            lyricsEntries = []
            lyricsAvailable = false
            lyricsFilePath = ""
        ...
        var sourceKey = src.toString()
        if (sourceKey === loadedLyricsSource)
            return
        loadedLyricsSource = sourceKey

        // File lookup/read runs on the bounded I/O pool. Clear the old track
        // immediately and apply only the result that still matches sourceKey.
        lyricsEntries = []
        lyricsAvailable = false
        lyricsFilePath = ""
        currentLyricIndex = -1
        musicLib.loadLyricsTextAsync(sourceKey)
    }

    function applyLyricsResult(raw, lrcFileUrl) {
        if (raw && raw.length > 0) {
    ...
    // Decode bounded cover textures off the GUI thread to avoid frame stalls.
    ...
    Item {
        ...
        Column {
            ...
            ListView {
                ...
                model: lyricsAvailable
                       ? lyricsEntries
                       : [ { text: theme ? theme.localized("No lyrics found", "未找到歌词") : "No lyrics found" } ]
                currentIndex: currentLyricIndex
                reuseItems: true
                // 高亮移动与范围：将当前行保持在视图偏上的位置
                highlightMoveDuration: theme && theme.reducedMotion ? 0 : 280
                ...
                delegate: Item {
                    ...
                    Connections {
                        target: root
                        // A lyric change should not wake every cached row. The
                        // list highlight handles distant rows; only the
                        // current line and its nearby context need the Apple-
                        // style offset animation.
                        enabled: lyricList.visible && Math.abs(index - currentLyricIndex) <= 8
                    ...
                    SequentialAnimation {
                        ...
                        PauseAnimation {
                            duration: theme && theme.reducedMotion ? 0 : recoverAnim.delayDuration
                        }
                        NumberAnimation {
                            target: lyricTrans
                            property: "y"
                            to: 0
                            duration: theme && theme.reducedMotion ? 0 : 280
                    ...
                    Item {
                        ...
                        Behavior on opacity {
                            NumberAnimation {
                                duration: theme && theme.reducedMotion ? 0 : 180
                                easing.type: Easing.OutCubic
                            }
                        }
                        // 放大动画使当前行视觉更突出
                        ...
                        Item {
                            ...
                            readonly property real lineEnd: (index + 1 < lyricsEntries.length)
                                                            ? lyricsEntries[index + 1].t
                                                            : (sourceItem && sourceItem.duration > 0
                                                               ? sourceItem.duration * 1000
                                                               : lineStart + 3000)
                            // Avoid subscribing inactive rows to the player's
                            // high-frequency position signal.
                            readonly property real posMs: (isActive && sourceItem) ? sourceItem.positionMs : 0
            ...
            Connections {
                ...
                function onSongTitleChanged() {
                        title = localizedSongTitle(sourceItem.songTitle)
                }
                function onArtistNameChanged() {
                        artist = localizedArtistName(sourceItem.artistName)
                }
        ...
        Item {
            ...
            Row {
                ...
                Text {
                    ...
                    MouseArea {
                        ...
                        Accessible.name: theme ? theme.localized("Previous track", "上一曲") : "Previous track"
                        onClicked: {
                ...
                Text {
                    ...
                    MouseArea {
                        ...
                        cursorShape: Qt.PointingHandCursor
                        Accessible.role: Accessible.Button
                        Accessible.name: sourceItem && sourceItem.isPlaying
                                         ? (theme ? theme.localized("Pause", "暂停") : "Pause")
                                         : (theme ? theme.localized("Play", "播放") : "Play")
                        onClicked: {
                            if (!sourceItem) return
                            if (sourceItem.isPlaying) {
                                if (sourceItem.pauseClicked) sourceItem.pauseClicked()
                ...
                Text {
                    ...
                    MouseArea {
                        ...
                        cursorShape: Qt.PointingHandCursor
                        Accessible.role: Accessible.Button
                        Accessible.name: theme ? theme.localized("Next track", "下一曲") : "Next track"
                        onClicked: {
            ...
            Rectangle {
                ...
                MouseArea {
                    ...
                    cursorShape: Qt.PointingHandCursor
                    Accessible.role: Accessible.Button
                    Accessible.name: theme && theme.musicVolume === 0
                                     ? (theme ? theme.localized("Unmute", "取消静音") : "Unmute")
                                     : (theme ? theme.localized("Mute", "静音") : "Mute")
            ...
            Text {
                ...
                MouseArea {
                    ...
                    Accessible.name: theme ? theme.localized("Volume", "音量") : "Volume"
                    onClicked: {
                        if (!theme) return
    ...
    Connections {
        ...
        function onEffectiveLanguageChanged() {
            root.title = root.localizedSongTitle(root.title);
            root.artist = root.localizedArtistName(root.artist);
        }
    }
