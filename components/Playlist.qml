...
Item {
    ...
    property int prefetchHead: 0
    property int maxPrefetchQueue: 128
    property var queuedPrefetchSources: ({})
    property var sourceIndex: ({})
    property var pendingFiles: []
    property int pendingFileIndex: 0
    property int fileBatchSize: 64
    property bool singleFolderMode: false
    property bool initialPrefetchDone: false
    property var removedSources: []

    // Metadata reads are serialized and limited to rows near the viewport.
    Timer {
        ...
        running: false
        onTriggered: {
            if (prefetchHead >= prefetchQueue.length) {
                prefetchQueue = []
                prefetchHead = 0
                running = false
                return
            }
            var next = prefetchQueue[prefetchHead++]
            delete queuedPrefetchSources[next]
            if (next) musicLib.prefetchMetadata([next])
            // Avoid O(n) shift() calls and periodically release consumed rows.
            if (prefetchHead > 32 && prefetchHead * 2 >= prefetchQueue.length) {
                prefetchQueue = prefetchQueue.slice(prefetchHead)
                prefetchHead = 0
            }
        }
    }
    ...
    Timer {
        id: visiblePrefetchTimer
        interval: 120
        repeat: false
        onTriggered: root.scheduleVisiblePrefetch()
    }

    // Model notifications and delegate creation stay bounded per frame.
    Timer {
        id: filePopulateTimer
        interval: 16
        repeat: true
        onTriggered: root.populateFileBatch()
    }

    function schedulePrefetch(srcs) {
        if (!srcs || srcs.length === 0) return
        for (var i = 0; i < srcs.length; i++) {
            var sourceKey = String(srcs[i] || "")
            if (sourceKey.length === 0 || queuedPrefetchSources[sourceKey])
                continue
            if (prefetchQueue.length - prefetchHead >= maxPrefetchQueue)
                break
            queuedPrefetchSources[sourceKey] = true
            prefetchQueue.push(sourceKey)
        }
    ...
    function appendUniqueSources(srcs) {
        ...
        for (var existingIndex = 0; existingIndex < fileModel.count; ++existingIndex) {
            var existing = fileModel.get(existingIndex)
            if (existing && existing.source)
                known[String(existing.source)] = true
        }
        for (var i = 0; i < srcs.length; i++) {
            ...
            if (s && !known[s]) {
                ...
                known[s] = true
                sourceIndex[s] = fileModel.count - 1
            }
        }
    }

    function setFiles(files) {
        filePopulateTimer.stop()
        sequentialPrefetchTimer.stop()
        prefetchQueue = []
        prefetchHead = 0
        queuedPrefetchSources = ({})
        pendingFiles = files ? Array.prototype.slice.call(files) : []
        pendingFileIndex = 0
        sourceIndex = ({})
        fileModel.clear()
        if (pendingFiles.length === 0)
            return
        filePopulateTimer.start()
    }

    function populateFileBatch() {
        var endIndex = Math.min(pendingFiles.length,
                                pendingFileIndex + Math.max(1, fileBatchSize))
        for (var i = pendingFileIndex; i < endIndex; ++i) {
            var s2 = pendingFiles[i]
            ...
            sourceIndex[s2] = fileModel.count - 1
        }
        pendingFileIndex = endIndex
        if (pendingFileIndex >= pendingFiles.length) {
            filePopulateTimer.stop()
            pendingFiles = []
            pendingFileIndex = 0
            visiblePrefetchTimer.restart()
        }
    }
    ...
    function rebuildSourceIndex() {
        ...
        for (var i = 0; i < fileModel.count; ++i) {
            ...
            if (item && item.source)
                nextIndex[String(item.source)] = i
        }
    ...
    function baseName(p) {
        if (!p || typeof p !== "string")
            return theme ? theme.localized("Unknown", "未知") : "Unknown"
        var seg = p.split(/[\\/]/).pop()
    ...
    Column {
        ...
        Text {
            id: titleText
            text: theme ? theme.localized("Playlist", "播放列表") : "Playlist"
            font.pixelSize: 18
            font.bold: true
        ...
        ListView {
            ...
            reuseItems: true
            // Keep a small reserve for smooth wheel scrolling without
            // constructing an entire off-screen playlist.
            cacheBuffer: Math.min(480, Math.max(160, height))
            clip: true
            model: fileModel
            ...
            highlight: Item {
                ...
                Behavior on y { enabled: !(theme && theme.reducedMotion); NumberAnimation { duration: theme && theme.reducedMotion ? 0 : 320; easing.type: Easing.OutCubic } }
                Behavior on height { enabled: !(theme && theme.reducedMotion); NumberAnimation { duration: theme && theme.reducedMotion ? 0 : 180; easing.type: Easing.OutCubic } }
                SequentialAnimation {
            ...
            // Do not animate contentY: it is driven by Flickable physics and
            // an additional Behavior makes drag/wheel input feel delayed.
            onContentYChanged: visiblePrefetchTimer.restart()
            ...
            delegate: Item { id: rowItem
                ...
                Row {
                    ...
                    Column {
                        ...
                        Text {
                            id: artistLabel
                            text: (itemObj && itemObj.artist && itemObj.artist.length > 0)
                                  ? ((itemObj.artist === "未知艺术家" || itemObj.artist.toLowerCase() === "unknown artist")
                                     ? (theme ? theme.localized("Unknown artist", "未知艺术家") : "Unknown artist")
                                     : itemObj.artist)
                                  : (theme ? theme.localized("Unknown artist", "未知艺术家") : "Unknown artist")
                            font.pixelSize: 12
        ...
        Item {
            ...
            Rectangle {
                ...
                Text { anchors.centerIn: parent; text: theme ? theme.localized("No music files found", "未找到音乐文件") : "No music files found"; font.pixelSize: 14; color: theme ? theme.textColor : "#ffffff" }
            }
        }
        Connections {
            target: musicLib
            function onMetadataReady(src, meta) {
                var index = sourceIndex[String(src)]
                if (index === undefined || index < 0 || index >= fileModel.count)
                    return
                var it = fileModel.get(index)
                if (!it)
                    return
                fileModel.set(index, {
                    source: it.source,
    ...
    Component.onCompleted: {
        ...
        if (model && model.count && model.count > 0) {
            for (var i = 0; i < model.count; i++) {
                var sourceItem = model.get(i)
                var s = sourceItem && sourceItem.source ? sourceItem.source : ""
                if (!s) continue
                fileModel.append({
                    source: s,
                    title: sourceItem.title || baseName(s),
                    artist: sourceItem.artist || ""
                })
                sourceIndex[s] = fileModel.count - 1
            }
        } else if (root.autoScan) {
            // Results arrive through musicFilesChanged without blocking the
            // scene graph while the filesystem is traversed.
            musicLib.scanAllAvailableMusicAsync(true)
            musicLib.startWatching()
        }
        // Metadata is intentionally limited to the initial viewport. The
        // scroll handler schedules later rows through a debounced timer.
        visiblePrefetchTimer.restart()
    }

    FileDialog {
        id: importFilesDialog
        title: theme ? theme.localized("Select music files", "选择音乐文件") : "Select music files"
        fileMode: FileDialog.OpenFiles
        nameFilters: [(theme ? theme.localized("Audio files", "音频文件") : "Audio files") + " (*.mp3 *.m4a *.flac *.wav *.ogg *.aac *.wma)"]
        onAccepted: {
            var arr = []
    ...
    FolderDialog {
        ...
        title: theme ? theme.localized("Select music folder", "选择音乐文件夹") : "Select music folder"
        onAccepted: {
            var url = "" + (importFolderDialog.selectedFolder || "")
            removedSources = []
            filePopulateTimer.stop()
            pendingFiles = []
            pendingFileIndex = 0
            ...
            musicLib.scanOnlyDirectoryAsync(url)
        }
    }
    ...
    Item {
        ...
        Column {
            ...
            Rectangle {
                ...
                Text { anchors.centerIn: parent; text: theme ? theme.localized("Import files", "导入文件") : "Import files"; font.pixelSize: 14; color: theme ? theme.textColor : "#ffffff" }
                MouseArea {
                    anchors.fill: parent
                    hoverEnabled: true
            ...
            Rectangle {
                ...
                color: "transparent"
                border.color: "transparent"
                Text { anchors.centerIn: parent; text: theme ? theme.localized("Choose folder", "选择文件夹") : "Choose folder"; font.pixelSize: 14; color: theme ? theme.textColor : "#ffffff" }
                MouseArea {
                    anchors.fill: parent
                    hoverEnabled: true
                    onEntered: parent.color = (theme ? Qt.rgba(theme.focusColor.r, theme.focusColor.g, theme.focusColor.b, 0.16) : Qt.rgba(0,196,179,0.16))
    ...
    Connections {
        ...
        function onMusicFilesChanged(newFiles) {
            setFiles(newFiles)
            if (singleFolderMode && playerRef && typeof playerRef.resetSources === 'function')
                playerRef.resetSources(newFiles)
        }
    }
